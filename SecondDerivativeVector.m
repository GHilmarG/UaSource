function [Deriv,detJ]=SecondDerivativeVector(coordinates,connectivity,nip,Iint)
    
    % calculates the second derivatives of form functions with respect to x and y and the Jacobian
...
% 
   ...
   % N=N(r,s) 
    ...
    % Second derivative:
    % x=x_p N_p(r,s) , y=y_p N_p(r,s) , h=h_p N_p(r,s)
    % ddh/ddx=d/dx( dh/dr dr/dx+ dh/ds ds/dx )  
    %         =d/dx( dh/dr dr/dx+ dh/ds ds/dx ) 
    %         = (dr/dx  1/dr + ds/dx 1/ds) ( dh/dr dr/dx+ dh/ds ds/dx ) 
    %         = (dr/dx  1/dr + ds/dx 1/ds) ( dh/dr dr/dx+ dh/ds ds/dx ) 
    %         =  ddh/ddr dr/dx  dr/dx + ddh/(dr ds)  dr/dx ds/dx + ddh/(ds dr) ds/dx dr/dx + ddh/dds ds/dx ds/dx 
    %         =  [dr/dx  ds/dx]  [ddh/ddr  ddh/(dr ds) ]   [dr/dx  ds/dx]'
    %                            [ddh/(ds sr)  ddh/dds ]
    %
    %
    %  [ddh/ddx  ddh/dxdy] =  [dr/ds  ds/dx] [ddh/ddr  ddh/(dr ds) ] [dr/dx ds/dx]'
    %  [ddh/dydx ddh/ddy ]    [dr/dy  ds/dy] [ddh/(ds dr) ddh/dds  ] [dr/dy ds/dy] 
    %
    %  where ddh/ddr=  h_p ddN_p/ddr and p=1..nod  (over the nodes of an element)
    %
    % h_p (dN_p/dr dr/dx + dN_p/ds ds/dx)
    % ddh/ddy=dh/dr dr/dy+ dh/ds ds/dy =  h_p (dN_p/dr dr/dy + dN_p/ds ds/dy)
    %
    % or
    %
    % [dh/dx]= h_p [dr/dx   ds/dx ] [dN_p/dr] 
    ...
    if any(detJ<0) ; fprintf('derivVector: detJ is negative \n ') ;  end
