...
function [p,y,info] = TwoDSubspaceTrustRegionGram(g,H,v1,v2,Delta,Gram)




%TWODSUBSPACETR  Exact solution of the trust-region subproblem restricted
%   to the 2D subspace spanned by v1 and v2.
%
%   min_p   m(p) = g'*p + (1/2) p'*H*p     s.t.  ||p||_G <= Delta
%   p in span{v1,v2},   ||p||_G^2 = p'*G*p
%
%   [p,y,info] = TwoDSubspaceTR(g,H,v1,v2,Delta,G)
%
%   INPUTS
%     g     : n-by-1 gradient (the l^2/coefficient gradient, g_{l2})
...
%     Delta : trust-region radius, measured in the G-metric:
%             ||p||_G = sqrt(p'*G*p) <= Delta
%     G     : (optional) n-by-n SPD metric matrix defining the norm used
%             for the trust-region constraint, e.g. the FE mass matrix M
%             (for an L^2 trust region) or M+beta*K (for an H^1-type
%             trust region -- see BuildTRMetric.m). May be passed as a
%             numeric matrix or a function handle Gfun(x) returning G*x.
%             Default: [] / omitted -> G = I (plain Euclidean/l^2 norm,
%             i.e. the original behaviour of this function).
%
%             IMPORTANT (metric consistency): if v1 was built as a
%             steepest-descent direction with respect to G, i.e.
%             v1 = -G^{-1}*g_{l2}  (e.g. G=M gives the L^2 gradient,
%             G=M+beta*K gives an H^1-type gradient), then the SAME G
%             should be passed here. Only then is v1 the true
%             steepest-descent direction with respect to the norm
%             actually being constrained, which is what the standard
%             Cauchy-decrease / global convergence guarantees require.
%
%   OUTPUTS
...
%   minimisation, generalised to an arbitrary SPD trust-region metric G
%   (G=I recovers the standard Euclidean-norm version). The (generally
%   non-orthogonal, non-G-orthogonal) basis is whitened using the
%   G-Gram matrix so that the norm constraint becomes a standard
%   Euclidean ball in the whitened coordinates, the reduced 2x2 Hessian
%   is diagonalised, and the boundary solution is obtained as the root
%   of a quartic secular equation. Candidates are selected by direct
%   evaluation of the reduced quadratic model, which is more robust
%   than trying to reason about "the" correct root analytically.
...
% ---- trust-region metric: M = V'*G*V, G defaults to I -----------
if isempty(Gram)
    GV = V;                      % G = I
elseif isa(Gram,'function_handle')
    GV = [Gram(v1), Gram(v2)];
else
    GV = Gram*V;
end
M = V'*GV;                       % 2x2 Gram matrix in the G-metric
M = (M+M')/2;
