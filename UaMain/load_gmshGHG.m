function mesh = load_gmshGHG(filename)
% Reads a mesh in msh format, version 1 or 2
%
% Copyright (C) 10/2007 R Lorphèvre (r.lorphevre@ulg.ac.be)
%
% however as the version on the web did not work!!! I did some
% changes to the origianl m file (Hilmar)
%
...
while 1
    ...
    if tline(2) == 'N' && ( tline(3) == 'O' || tline(3) == 'o' )
        ...
        IDS=zeros(mesh.nbNod,1);  % GHG
        for I=1:mesh.nbNod
            iNod = fscanf(fid, '%d', 1);
            X = fscanf(fid, '%g', 3);
            IDS(iNod) = I;
            % [- GHG commented out and replaced with a vector operation
            %                 if (I == 1)
            %                     mesh.MIN = X;
            %                     mesh.MAX = X;
            %                 else
        ...
        % [GHG
        mesh.MAX(1)=max(mesh.POS(:,1)) ;
        mesh.MAX(2)=max(mesh.POS(:,2)) ;
        mesh.MAX(3)=max(mesh.POS(:,3)) ;
        mesh.MIN(1)=min(mesh.POS(:,1)) ;
        mesh.MIN(2)=min(mesh.POS(:,2)) ;
        mesh.MIN(3)=min(mesh.POS(:,3)) ;
        % -]
        tline = fgetl(fid);
        fprintf(' done. ')
