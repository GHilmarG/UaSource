...
function HPsiddFdCdC=Psi_d2Fdpdq_xi(CtrlVar,MUA,F,uAdjoint,vAdjoint)
...
% then
%
% $$ [H^{\#8}]^T=H^{\#8}_{ji}=\Psi_n \frac{\partial^2 F_n}{\partial q_k \, \partial p_i} \xi_{kj} =\Psi_n \frac{\partial^2 F_n}{\partial p_i \,\partial q_k} \xi $$
%
%
%
...
% we have
%
% $$\delta_C F_x = -\frac{\mathcal{G}}{m} \,  (C+C_0)^{-1/m-1} \; (v_x^2+v_y^2 + v_0)^{(1-m)/2m} \, v_x \; \delta C$$
%
% and
%
% $$\delta^2_{v_x \,C } F_x = -\frac{\mathcal{G}}{m} \,  (C+C_0)^{-1/m-1} \; \left  (\frac{1-m}{2m} (v_x^2+v_y^2 + v_0)^{(1-3m)/2m} \, v_x +  (v_x^2+v_y^2 + v_0)^{(1-m)/2m} \right )    \; \delta C \, \delta u$$
%
%
%
% $$
...
%
% $$ \langle \Psi | \delta^2_{v_x C} F \rangle =  
%  -\int   \frac{\mathcal{G}}{m} \,  (C+C_0)^{-1/m-1} \; \left  (\frac{1-m}{2m} (v_x^2+v_y^2 + v_0)^{(1-3m)/2m} \, v_x +
%  (v_x^2+v_y^2 + v_0)^{(1-m)/2m} \right )    \; \phi_i \, \phi_j \; dx \, dy
% $$
% 
