...
if isnan(t)
 ...
 %t1=U.*gamm.*(1i.*k.*U.*nu-ca.*H.*l.^2.*tau+expp.*(p.*nu-1i.*k.*U.*nu+ca.*H.*l.^2.*tau));
 %t2=p.*(gamm+H.*j2.*eta).*xi;

t1=U.*gamm.*(1i.*k.*U.*(m.*gamm+H.*(k.^2+4.*l.^2).*eta)-ca.*H.*l.^2.*tau+expp.*((p-1i.*k.*U).*(m.*gamm+H.*(k.^2+4.*l.^2).*eta)+ca.*H.*l.^2.*tau));
t2=p.*ps;
