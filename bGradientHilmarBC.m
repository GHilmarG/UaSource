function [bGrad]=bGradientHilmarBC(u,v,s,h,etaInt,gfint,rho,rhow,g,alpha,m,C,Lambda,Mu,connectivity,coordinates,nip)
    
    % calculates \lambda_q \p r_q/\p I =Lambda_q \frac{\p}{\b_i}  <L u-f| N_q> 
    % where the FE formulation used has Weertman BC and the natural boundary condition
    ...
    for Iele=1:Nele
        ...
        h_l=h(con);  u_l=u(con); v_l=v(con) ; s_l=s(con);
        
        
        gx_l=con; Fx=zeros(nod,nod) ;Fy=Fx;
        ...
        for Iint=1:nip                           % loop over integration points
            ...
            Fx=Fx+...
                ((4*dudxI+2*dvdyI)*etaI*deriv(1,:)'*fun'+(dvdxI+dudyI)*etaI*deriv(2,:)'*fun'+...
                rho*g*(deriv(1,:)*s_l*ca-sa)*(fun*fun')-...
                varrho*g*ca*(fun*(fun' * dhdxI+ hI * deriv(1,:))+hI*deriv(1,:)'*fun'))*detJw;
            
            Fy=Fy+...
                ((4*dvdyI+2*dudxI)*etaI*deriv(2,:)'*fun'+(dudyI+dvdxI)*etaI*deriv(1,:)'*fun'+...
                rho*g*deriv(2,:)*s_l*ca*(fun*fun')-...
                varrho*g*ca*(fun*(fun' * dhdyI+ hI * deriv(2,:))+hI*deriv(2,:)'*fun'))*detJw;
    ...
    bGrad=[Lambda;Mu]'*[Bu';Bv']; 
    %bGrad=[Bu ;Bv]' *[Lambda;Mu];
    
    
    % Note : in the inner product notation: dA_{kl}/dq_i u_l= [Bu ; Bv]
    
    bGrad=bGrad(:); bGrad=real(bGrad);
    
    
    
    bGrad=EleAverageInterpolate(bGrad,coordinates,connectivity);
    
    %save savebGradientHilmarBC Lambda Mu Bu Bv coordinates connectivity
    ...
    if TestGradient==1
        ...
        for I=1:Nnodes
            ...
            bGradNumeric(I)=-[Lambda;Mu]'*DkvDb*[u;v]+[Lambda;Mu]'*DrhDb;  % var her: think anout if the second term should have a minus sign 
        ...
        figure(700) ; trisurf(TRI,x,y,bGradNumeric) ;  title(' bGradNumeric ')
        figure(701) ; trisurf(TRI,x,y,bGrad) ;  title(' bGrad ')
        figure(702) ; trisurf(TRI,x,y,bGradNumeric-bGrad) ;  title(' bGradNumeric-bGrad ')
        figure(703) ; trisurf(TRI,x,y,bGradNumeric1) ;  title(' bGradNumeric1 ')
        figure(704) ; trisurf(TRI,x,y,bGradNumeric2) ;  title(' bGradNumeric2 ')
        
        figure(705) ; trisurf(TRI,x,y,bGradNumeric1-bGrad) ;  title(' bGradNumeric1-bGrad ')
        figure(706) ; trisurf(TRI,x,y,bGradNumeric2-bGrad) ;  title(' bGradNumeric2-bGrad ')
        
        save Testdb2 TRI x y bGrad bGradNumeric
