...
function [UserVar,rh]=dIhdotduv(UserVar,CtrlVar,MUA,F,dhdtres)
...
for Iint=1:MUA.nip
    ...
    for Inod=1:MUA.nod
        ...
        termx=-dhdtresint.*(dhdx.*fun(Inod)+hint.*Deriv(:,1,Inod)).*detJw;
        termy=-dhdtresint.*(dhdy.*fun(Inod)+hint.*Deriv(:,2,Inod)).*detJw;
