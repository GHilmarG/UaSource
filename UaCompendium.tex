...
in this case simply
\begin{equation} 
4 \p_x ( h \eta \p_x u ) =\rho g (1-\rho/\rho_w) h \, \p_x h  ,
\label{eq:mbic1}
\end{equation}
...
\begin{equation} 
2 \p_x ( h \tau_{xx} ) =\rho g (1-\rho/\rho_w) h \, \p_x h  ,
\label{eq:1dstressf}
\end{equation}
or as
\begin{equation} 
...
\begin{equation}
\underbrace{\int_{b}^{s} \sxx \, dx }_{\hbox{ocean}}=-\int_{b}^{S} \rho_w g (S-z) \, dz = -\frac{1}{2} \rho_w g (S-b)^2,
\label{eq:pshelf}
\end{equation}
...
\begin{equation}
2 h \tau_{xx} - \frac{1}{2} \rho g h^2 = -\frac{1}{2} \rho_w g  (S-b)^2 ,
\label{eq:2ht}
\end{equation}
Using Eq.~(\ref{eq:2ht}) together with the floating condition (\ref{eq:bh1}) we find
\begin{eqnarray}
2 h \tau_{xx} &=& \frac{1}{2} \rho g h^2 - \frac{1}{2} \rho_w g  (S-b)^2 \nonumber \\
          &=& \frac{1}{2} \rho g h^2 - \frac{1}{2} \rho_w g   h  (S-b)\frac{\rho}{\rho_w} \nonumber \\ 
...
\begin{equation}
2 A^{-1/n} h | \partial_x u|^{(1-n)/n} \, \partial_x u = \frac{1}{2} \rho g (1-\rho/\rho_w) h^2
\label{eq:bcstressu}
\end{equation}
and
\begin{equation}
8 \eta h \partial_x u -\rho g (1-\rho/\rho_w) h^2 =0 ,
\label{eq:bcstressu3}
\end{equation}
at $x=x_c$.  
...
\begin{equation}
4 \partial_x (\eta h \partial_x u ) = \rho g (1-\rho/\rho_w) h \partial_x h .
\label{eq:bcstressu4}
\end{equation}
...
$\sxx=-\rho (1-\rho/\rho_w) g h /2$, and is negative.
...
\caption{\label{fig:stresses}Left: Stresses within a one-dimensional
  ...
  $\rho_w=1030 \, \mathrm{kg \, m^{-3}}$, $h= 100 \, \mathrm{m}$.}
\end{figure}
...
\[
 \sigma_{xx}+p_o= \rho g (s/2-z) + \rho_w z
\]
...
% \[\rho (\p_t h + \p_{x_{gl}} h \, \p_t x_{gl} ) = -\rho_w (\p_t b + \p_{x_{gl}} b \, \p_t x_{gl} ) 
% \]
% or
% \[
% \p_t x_{gl} = -\frac{rho_w \p_t b + \rho \p_t h}{\rho_w \p_x b + \rho \p_x h}
% \]
...
% \[
% \p_t x_{gl} = -\frac{rho_w \p_t b + \rho (a-\p_x q}{\rho_w \p_x b + \rho \p_x h}
% \]
% 
% 
...
\caption{\label{fig:alp}Analytical ice shelf profile.  The left hand figure is for an
  ...
  $u_{gl}=300\,\mathrm{m\,a^{-1}}$, $\rho=910 \, \mathrm{kg \,
    m^{-3}}$, $\rho_w=1030 \, \mathrm{kg \, m^{-3}}$. The value for
...
\rho_w H$, where $H=S-B$ with $S$ the ocean surface and $B$ the ocean
bed.
...
% line the floating condition \[ \rho (s-b)=\rho_w (S-b) \quad \hbox{for} \quad x \ge \xgl,\]
% dictates that surface and thickness slopes are related through
% \begin{align*}
% \p_x h & = -\rho_w \p_x b /\rho ,\\
% \p_x s & = - (\rho_w/\rho -1) \, \p_x b ,\\
...
\begin{equation}
   u = C \left (\rho g h h A \left (\rho g (1-\rho/\rho_w) g h/4 \right )^n /u \right )^m .
\label{eq:uch2}
\end{equation}

%or
%\[
%u^{m+1}=  4^{-n m}\, C \,  A^m (g \rho)^{m+n m} (1-\rho/\rho_w)^{n m}  \;  h^{n m + 2 m} ,
%\]
