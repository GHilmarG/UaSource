...
\end{equation}
We can therfore also write the field equations as 
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:idder}
\end{equation}
which is identical to (\ref{eq:dder}) which we dervied earlier using a
somewhat different approach.\footnote{Apparently the 'adjoint' method
  gets its name from the fact that the resulting expressoin of the
...
\begin{enumerate}
\item The directional derivative of the forward model with respect to the state and control variables, i.e $\p_q F$ and $\p_p F$.
\item The directional derivative of the objective function with respect to the state and control variables, i.e.\ $\p_q J$ and $\p_p J$.
\end{enumerate}
...
\section{Simple example of a gradient calculation of an objective funcion}
...
\begin{align*}
  F &= \int \left (\dot{h}  - \left ( a - \p_x ( u h ) \right ) \right ) \, \phi_j \; dA  \\
...
object funcion $J$ with respect to the control variable $h$, i.e.\
\[
