...
is needed frequently and we define $\G$ as
\[
\G=\He(h-h_f)
\]
Hence
...
$I$ & misfit term\\
${\cal L} = I + R + <\lambda \mid F(q(p),p) > $ & Lagrangian or the 'extended' objective function. \\
$K$ & covariance matrix \\
...
as
\begin{equation} 
{\cal L}(q(p),p,\lambda)=J(q(p),p)+<\lambda \mid F(q(p),p) >_{W^\ast ,W}
\label{eq:L1}
\end{equation}
...
Introducing
\[
j(u)=J(q(p),p)= {\cal L}(q(p),p,\lambda)
\]
we have
\begin{equation}
j'(p)= (\p q / \p p )^\ast \p {\cal L}(q(p),p,\lambda)/\p q + \p {\cal L}(q(p),p,\lambda)/ \p p
\label{eq:dj1}
\end{equation}
Now we chose $\lambda$ such that 
\[
\p {\cal L}(q(p),p,\lambda)/ \p q=0
\]
...
\[
< j'(p), \phi>_{P^\ast, P}= < \p_u {\cal L} , \p_p q \,  \phi >_{U^\ast , U}  + < \p_p {\cal L} , \phi>_{P^\ast,P}  
\]
and the directional derivative is then 
\[
<\p_u {\cal L}, \p_p q \, \phi>_{U^\ast, U}=0
\]
for all $\phi$.
...
\begin{enumerate}
\item The derivative of the forward model with respect to the state and control variables, i.e $\p_q F$ and $\p_p F$.
\item The derivative of the objective function with respect to the state and control variables, i.e.\ $\p_q J$ and $\p_p J$.
\end{enumerate}
...
\[
\min_{p} I(q(p)) = < q -\tilde{q} \mid K_q^{-1} \mid u -\tilde{u} > + < p -\tilde{p} \mid K_P^{-1} \mid p -\tilde{p} >  \\
\]
we would arrive at 
\[
...
\[
I_u=<C^{-1/2}_{uu} u-\tilde(u) , C^{-1/2}_{uu} u-\tilde(u) >
\]
