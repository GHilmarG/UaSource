...
A bit more precise notation is:

\begin{align}
   < \delta_q F \mid \Delta q > & = -<F(q) \mid \delta_q> \quad \text{ with } q \to q +\Delta q \text{ and } \Delta q \to 0  \\
  <\delta_q F^{*} \mid \lambda > & = \delta_q J \\
  \delta_p J &= < \p_p F^{*} \mid \lambda > + \delta_p J
\end{align}
...
thickness $h$ using rates of elevation as observations. Here the
control variable is $h$ and the field variable $\dot{h}$. We have
measurements $\tilde{\dot{h}}$ and an a priori estimate $\tilde{h}$.
...
One can now ask the question what this directional derivative looks
like in a FE setting. Assuming that all variables are expanded in the
same FE basis, i.e.\ $f=f_i \phi_i$ where $f$ is any of  $u$, $h$, $\dot{h}$ and also
\[
\delta J = (\delta_h J)_i \phi_i
\]
we would solve for the coefficients $(\delta_h J)_i$ in the expansion
$\delta_h J = (\delta_h J)_i \phi_i$ in the usual FE manner by
writing\footnote{There appears to be some confusion in the literature
...
example of a FE assembly. It does note represent the values of the
...
$\phi_i$ this matrix is the mass matrix $[\bm{M}]_{ij}= < \phi_i \mid \phi_j > $.
...
$\bm{M} \bm{\lambda}$. Also note that the term $\p_{\dot{h}} J$ is a
directional derivative and, again in FE context, reads
\[
