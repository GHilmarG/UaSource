...
We will now derive Eq.~(\ref{eq:qUa1}) using thelocal form of the
...
\section{Inversion in \Ua}

Currently in \Ua it is possible to invert for $A$ and/or $C$, and bed
geometry ($B$) using measurements of horizontal surface velocities
$(u_s,v_s)$ and/or rates of thickness changes ($\p_t h$). One can
invert jointly for $A$ and $C$ or any combination there off such as
$\log A$ and $\log C$. But currently joint inversion with $B$ are not
implemented. When inverting for $A$ and $C$, regularization can be
applied on $A$, $C$ or $\log A$ and $\log C$. All combinations are
possible, so one can, for example, invert for $\log A$ and $C$ with
regularization applied on $A$ and $\log C$, if one so prefers. Typical
use involves inverting for $\log A$ and $\log C$ simultaneously with
regularization applied on $\log A$ and $\log C$. When inverting for
$B$, regularisation is applied on $B$. Note that regularization is
always applied on the difference between $p$ and its prior
$\tilde{p}$, i.e.\ on $p-\tilde{p}$.  In addition, strickt limits can
put on the range of the control parameter $p$. One can, for
example, but strickt upper and lower limits on $B$ at each location.

One can invert for $A$ and $C$ either over nodes or over
elements. Typically inversion is done over nodes. Inversion for $B$ is
done over nodes only.

The forward model is the SSTREAM momentum equation. Note that if one
includes $\dot{h}$ as measurements, in effect, the mass balance
equation is used as well. It is a question of viewpoint if the forward
model then includes the mass conservation in addition to the
conservation of momentum, or just the momentum equation with the mass
conservation used to calculate $\dot{h}$ as a part of the evaluation
of the cost function. Both viewpoints are mathematically equivalent.

\begin{table}[t]
\caption{\label{tab:UaInv} List of variables}
\begin{center}
\begin{tabular}{cccc}
\hline
Control variables ($p$) & regularisation &  mesurements ($\tilde{q}$) & forward model $F$ \\
\hline
$A$, $C$, $\log A$ , $\log C$   & $A$, $C$, $\log A$ , $\log C$   &  $u_s$, $v_s$, $\dot{h}$   &  SSTREAM (SSA) \\
$b$, $B$                        & $b$, $B$                        &  $u_s$, $v_s$, $\dot{h}$   &  SSTREAM (SSA) \\ 
\hline
\end{tabular}
\end{center}
\end{table}
...
the (total) directional derivative $d_p J$ of the objective function
$J$ with respect to the control parameters $p$, as follows:
...
\begin{align}
  ...
  d_p J &= < \delta_p F^{*} \mid \lambda > + \delta_p J
\end{align}
...
\label{eq:djH}
\end{equation}
This is the desired directional derivative and one can now do an
inversion for $h$ using this derivative.

Note that if we set $\delta h = \phi$, which is a typical approach and
the one used in \Ua, this is the derivative with respect to the
perturbation in the nodal values, i.e.\ $\delta h = \phi$. This
derivative is clearly intimately related to the FE basis. There is
then the separate, but related and quite an important question as to
what the {\em coefficients of the directional derivative} are in a
given FE setting. Assuming that all variables are expanded in the same
FE basis, i.e.\ $f(x,y)=f_i \phi_i(x,y)$ where $f$ is any of $u$, $h$,
$\dot{h}$ and also
\[
\delta_h J = (\delta_h J)_i \, \delta h_i = (\delta_h J)_i \, \phi_i
\]
we must solve for these coefficients, i.e.\ $(\delta_h J)_i$ in the expansion
...
structure of the FE approximation used, both the basis and the
...
\begin{align*}
\bm{F} & = \bm{M} \dot{\bm{h}} - \bm{b} , \\
J & = \frac{1}{2 \mathcal{A}} (\bm{\dot{h}}-\bm{\tilde{\dot{h}}})^T \bm{M}  (\bm{\dot{h}}-\bm{\tilde{\dot{h}}})
 + \frac{1}{2\mathcal{A}} (\bm{h}-\bm{\tilde{h}})^T \bm{M}  (\bm{h}-\bm{\tilde{h}}).
\end{align*}
...
\[
d_{h} J = < \delta_{h} F^{*} \mid \lambda > + \, \delta_{h} J .
\]
where the $\delta$ symbols indicates that these are directional derivative. This leads to
\begin{align*}
d_p J   & =  \int \lambda \, \p_x (u \, \delta h)  \; dA     + \frac{1}{\mathcal{A}} \int \left ( h - \tilde{h} \right ) \, \delta h \; dA , \\
        & = \frac{1}{\mathcal{A}} \int \left ( \dot{h}-\tilde{\dot{h}} \right ) \p_x (u \, \delta h)  \; dA     + \frac{1}{\mathcal{A}} \int \left ( h - \tilde{h} \right ) \, \delta h \; dA ,
\end{align*}
which is same as (\ref{eq:djH}). The second term on the right-hand
side can be evaluated directly from the discrete from as
\[
\delta_h J =
\frac{1}{\mathcal{A}} \bm{M}  (\bm{h}-\bm{\tilde{h}}).
\]
...
\section{Simplified gradients using a {\em fixpoint} approach.}

Experience has shown that it is sometimes usefull to start an inverse
iteration sequence with some ad-hoc estimation of the inverted field,
or some crude initial gradients. Such an approach can, for example, be
used to create a rough estimate of the inverted field directly from
measurments.

In \Ua the resulting gradients are referred to as {\em fixpoint
  gradients}. The argument for using these ad-hoc approaches, instead
of the correctly calculated gradients by the adjoint method, is that
they have shown to provide useful starting points for an inversion and
to speed up the initial steps of an inverse iteration.

Consider an inversion of $B$ and let's estimate what $B=s-h$ might look
like if we ignore horizontal stress transmission, i.e.
\[
u_s \approx C \tau^m ,
\]
with
\[
\tau \approx \rho g h \p_x s .
\]
A rough initial estimate $h_0$ for $h$ might be
\[
 h_0  = \frac{u}{C (\rho g \p_x s)^m} .
\]
This clearly has plenty of potential to go hopelessly wrong, but might
be OK as a rough initial estimate. A more robust approach would be to
approximate the gradient of the cost function
\[
J= \frac{1}{2} | u - \tilde{u} | + \frac{1}{2} | h -\tilde{h} | ,
\]
giving
\[
\delta_h J = (u-\tilde{u}) \frac{C m}{h} \tau^m + (h-\tilde{h}) ,
\]
where for notational simplicity we have assumed $| f | = f^2 $. This
is a much more robust approach as here $J$ is recalculated using the
forward model and the (incorrect) gradient is used in a robust
line-search step. This approach guarantees that the formally specified
misfit function is actually being minimized. At worst using this
approach the iteration will stagnate, in which case one simply
switches to the (correct) adjoint gradient.

A similar approach can be used to estimate $\delta_C J$, and using the
$C$ fixpoint gradient in \Ua for the first 1 or 2 iterations often results
quite dramatic reductions in the cost function. 
...
%\begin{eqnarray*} \p_x
%\sxx+\p_y \txy + \p_z \txz = 0 \\ \p_x
%\txy+\p_y \syy + \p_z \sigma_{yz} = 0 \\ \p_x
%\txz+\p_y \sigma_{yz} + \p_z \szz = 0 \\ 
%\end{eqnarray*}
...
% As a scale for the stresses we use
% \begin{equation}
%  [\sigma]=A^{-1/n} ([a]/[z])^{1/n} , \label{eq:stressscale}
% \end{equation} 
% which is motivated by the expectation $\exx \sim a/H$
% and Glen's flow law 
% \[
% \tau_{ij}=A^{-1/n} \,\dot{\epsilon}^{(1-n)/n}\, \dot{\epsilon}_{ij},
% \]
% where $\dot{\epsilon}$ is the effective strain rate defined through
% $\dot{\epsilon}:=(\dot{\epsilon}_{ij} \dot{\epsilon}_{ij}/2)^{1/2}$
% and $\tau_{ij}$ are the deviatoric stress components.   
...
\label{eq:usb}
\end{equation}
If we want the sliding velocity to be balanced by the basal shear stress, terms on both side of
...
\]
%\[
%\beta= \left (\frac{|\bm{v}_b|^{1-m}}{c} \right )^{1/2m}
%\]
...
% \begin{eqnarray*} \p_x
%\sxx+\p_y \txy + \p_z \txz = 0 \\ \p_x
%\txy+\p_y \syy + \p_z \syz = 0 \\ \p_x
%\txz+\p_y \syz + \p_z \szz = 0 \\ \end{eqnarray*}
