function [R,K,T,F]=KRTFuvhVer2(u,v,h,S,B,u0,v0,h0,as,ab,dt,AGlen,n,C,m,coordinates,connectivity,nip,alpha,rho,rhow,g,MeshProp,CtrlVar)


% checking if the missing dH/dx term in KRTFuvh changes anything
%s^{'}(x) =\He(h-h_f) (\rho/\rho_w h - H)
...
for Iint=1:nip
    ...
    for Inod=1:nod
        ...
        t1=-rhog.*hint.*(dsdotdx*ca-sa).*fun(Inod);
        ...
        t1=-rhog.*hint.*dsdotdy*ca.*fun(Inod);
