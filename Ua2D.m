...
if CtrlVar.InverseRun %  inverse run
    ...
    if CtrlVar.Restart %  forward restart run
        ...
        if ~(~CtrlVarInRestartFile.TimeDependentRun && CtrlVar.TimeDependentRun)  
            % restart file was NOT generated by a time independent run, while the current one is time dependent.
            % in that case, I replace dt with the value in the restart file
