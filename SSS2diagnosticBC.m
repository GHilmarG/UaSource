function [u,v,exx,eyy,exy,etaInt,xint,yint,kv]=...
        SSS2diagnosticBC(s,h,u,v,coordinates,connectivity,Nnodes,Nele,nip,nod,etaInt,AGlen,C,uzero,vzero,L,Lb,n,m,alpha,rho,rhow,g)
    ...
    beta2Imax=1e5; tol=1e-3 ;  etamax=1e20 ; % 
    %]-
    ...
    while (diff > tol  && iteration < iteration_max )  || iteration < iteration_min
        ...
        %rh=sparse(neq,1) ; kv=sparse(neq,neq)  ;
        ...
        if calcStrainRates==1
            ...
            for Iele=1:Nele                           % loop over elements
                ...
                con=connectivity(Iele,:);  % nodes of element
                coo=coordinates(con,:) ; % nod x dof =[x1 y1 ; x2 y2 ; x3 y3]
                h_l=h(con); s_l=s(con); u_l=u(con); v_l=v(con) ; 
                ...
                for Iint=1:nip                           % loop over integration points
                    ...
                    etaInt(Iele,Iint)=min([0.5*AGlen^(-1/n)*e^((1-n)/n),etamax]);
