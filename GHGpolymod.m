function [lplus]=GHGpolymod(q0, qp0, lamc, qc, blow, bhigh, lamm, qm)
    %
    ...
    if nargin == 6
        ...
        if (c(1)*c(1) - 3 *c(2) *qp0) < 0
            fprintf('warning: cubic fit gives a complex root  in GHGpolymod \n')
            if qm < qc ; lplus=lamc ; else lplus=lamm ; end
        else
        ...
        if c(2)==0 % perfect quadradic fit!!
            
            lplus = - qp0/(2 * lamc*(qc - q0 - qp0) );
            fprintf(' perfect quad fit giving lplus=%-g \n ',lplus)
            if lplus < lleft ;lplus = lleft; end
            if lplus > lright ;lplus = lright; end
        end
