...
for Inod=1:nod
    ...
    if ~Ronly
        for Jnod=1:nod
            ...
            % Ex = hint.*(4.*exx+2.*eyy).*Deriv(:,1,Inod)+2*hint.*exy.*Deriv(:,2,Inod);
            % Ey = hint.*(4.*eyy+2.*exx).*Deriv(:,2,Inod)+2*hint.*exy.*Deriv(:,1,Inod);
            ...
            Kxv(:,Inod,Jnod)=Kxv(:,Inod,Jnod)...
                +(etaint.*hint.*(2*Deriv(:,2,Jnod).*Deriv(:,1,Inod)+Deriv(:,1,Jnod).*Deriv(:,2,Inod))...
            ...
            Kxh(:,Inod,Jnod)=Kxh(:,Inod,Jnod)...
                ...
                +dtauxdh.*funI.*fun(Jnod)... 
                +2*fun(Jnod).*etaint.*(uint.*drhodx+vint.*drhody)./rhoint.*Deriv(:,1,Inod)... 
            ...
            Kyh(:,Inod,Jnod)=Kyh(:,Inod,Jnod)...
                ...
                +dtauydh.*funI.*fun(Jnod)...  
                +2*fun(Jnod).*etaint.*(uint.*drhodx+vint.*drhody)./rhoint.*Deriv(:,2,Inod)... 
