...
%
%  hdata.MeshSizeMax  = h0;                   Max allowable global element size.
...
%   options.dMeshSizeMax  : The maximum allowable (relative) gradient in the size 
%                    function { 0.3, 30.0% }.
...
tic
[qtree.p,qtree.t,qtree.h] = quadtreeMesh2d(node,edge,hdata,options.dMeshSizeMax,options.output);
t_quad = toc;
...
p = node;
e = edge;
i = tsearchGHG(ph(:,1),ph(:,2),th,p(:,1),p(:,2));               
...
if ~isempty(options)
   ...
   names = {'mlim','maxit','dMeshSizeMax','output'};
   ...
   if isfield(options,'dMeshSizeMax')                                             % Size function gradient limit
      checkposscalar(options.dMeshSizeMax,'options.dMeshSizeMax');
   else
      options.dMeshSizeMax = d_dMeshSizeMax;
   end
