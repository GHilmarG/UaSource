function [p,t,h] = quadtreeMesh2d(node,edge,hdata,dMeshSizeMax,output)
...
%  dMeshSizeMax : Maximum allowalble relative gradient in the size function
...
[MeshSizeMax,edgeh,fun,args] = gethdata(hdata);
...
pr = rotate(p,-theta);
h = userhfun(pr(:,1),pr(:,2),fun,args,MeshSizeMax,XYmin,XYmax);
...
while true                                                           
   ...
   h = [h; userhfun(pr(:,1),pr(:,2),fun,args,MeshSizeMax,XYmin,XYmax)];
...
while true                                                                 % Loop over the edges of the background mesh ensuring
   h_old = h;                                                              % that dh satisfies the dMeshSizeMax tolerance
   for k = 1:ne                                                            % Loop over edges
      n1 = e(k,1);
      n2 = e(k,2);
      Lk = L(k);
      if h(n1)>h(n2)                                                       % Ensure grad(h)<=dMeshSizeMax
         dh = (h(n1)-h(n2))/Lk;
         if dh>dMeshSizeMax
            h(n1) = h(n2) + dMeshSizeMax*Lk;
         end
      else
         dh = (h(n2)-h(n1))/Lk;
         if dh>dMeshSizeMax
            h(n2) = h(n1) + dMeshSizeMax*Lk;
         end
      end
   end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function h = userhfun(x,y,fun,args,MeshSizeMax,xymin,xymax)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [MeshSizeMax,edgeh,fun,args] = gethdata(hdata)
...
if ~isempty(hdata)
   ...
   names = {'MeshSizeMax','edgeh','fun','args'};
   for k = 1:length(fields)
      if ~any(strcmp(fields{k},names))
         error('Invalid field in HDATA');
      end
   end
   if isfield(hdata,'MeshSizeMax')
      if (numel(hdata.MeshSizeMax)~=1) || (hdata.MeshSizeMax<=0)
         error('HDATA.HMAX must be a positive scalar');
      else
         MeshSizeMax = hdata.MeshSizeMax;
      end
   else
      MeshSizeMax = d_MeshSizeMax;
   end
