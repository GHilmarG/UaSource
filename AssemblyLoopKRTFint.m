...
function   [Tx,Fx,Ty,Fy,Th,Fh,Kxu,Kxv,Kyu,Kyv,Kxh,Kyh,Khu,Khv,Khh]=...
            AssemblyLoopKRTFint(Iint,ndim,MUA,...
            bnod,hnod,unod,vnod,C,h0nod,u0nod,v0nod,as0nod,ab0nod,as1nod,ab1nod,dudtnod,dvdtnod,Bnod,Snod,rhonod,...
            CtrlVar,rhow,g,m,etaInt,exx,eyy,exy,Eint,Ronly,ca,sa,dt,...
            Tx,Fx,Ty,Fy,Th,Fh,Kxu,Kxv,Kyu,Kyv,Kxh,Kyh,Khu,Khv,Khh)
        
%%
    theta=CtrlVar.theta;
    Gamma=CtrlVar.TG3*dt/12;
...
%        h0barr=gamma_h*exp(arg0)/lambda_h;
    ...
    % currently evaluating du1dt in the same way as du0dt
    % I could get rid of the corresponding terms on the rhs as they cancel out
    du1dt=(uint-u0int)/dt; dv1dt=(vint-v0int)/dt;
            ...
            for Jnod=1:MUA.nod
             ...
             Kxh(:,Inod,Jnod)=Kxh(:,Inod,Jnod)...
                ...
                Khh(:,Inod,Jnod)=Khh(:,Inod,Jnod)...
                    ...
                    + Gamma*(du1dt.*Deriv(:,1,Inod)+dv1dt.*Deriv(:,2,Inod)).*fun(Jnod)...
                    ).*detJw;
...
%         fh=  ((1-theta)*a0int+theta*a1int).*fun(Inod);
        ...
        TG3rhs=...
            -Gamma*(uint.*Deriv(:,1,Inod)+vint.*Deriv(:,2,Inod)).*(a1int-qx1dx-qy1dy)...
            +Gamma*(u0int.*Deriv(:,1,Inod)+v0int.*Deriv(:,2,Inod)).*(a0int-qx0dx-qy0dy)...
            -Gamma*hint.*(du1dt.*Deriv(:,1,Inod)+dv1dt.*Deriv(:,2,Inod))...
            +Gamma*h0int.*(du0dt.*Deriv(:,1,Inod)+dv0dt.*Deriv(:,2,Inod));
        %
