function [u,v,exx,eyy,exy,etaInt,beta2,xint,yint,kv,lambdau,rh]=...
        SSS2diagnosticBCsparse(s,h,u,v,coordinates,connectivity,Nnodes,Nele,nip,nod,etaInt,gfint,AGlen,C,...
   ...
   %if any(h<0) ; error(' thickness negative ') ; end
    ...
    while (diff > tol  && iteration < iteration_max )  || iteration < iteration_min
        ...
        %TRI = delaunay(x,y);
        %figure; trimesh(TRI,x,y,u); title('u (trimesh)') ; xlabel('x') ; ylabel('y')
        %figure; trimesh(TRI,x,y,v); title('v (trimesh)') ; xlabel('x') ; ylabel('y')
        
        
        
        %diff=(max(abs(u-ulast))+max(abs(v-vlast)))/(max(abs(u))+max(abs(v)));
        % someting strainged about v
        D=mean(abs(u.*u+v.*v));
        ...
        if calcStrainRates==1
            ...
            for Iele=1:Nele                           % loop over elements
                ...
                for Iint=1:nip                           % loop over integration points
                    ...
                    e=sqrt(e00+exx(Iele,Iint)^2+eyy(Iele,Iint)^2+exx(Iele,Iint)*eyy(Iele,Iint)+exy(Iele,Iint)^2); % consider taking out of loop
            ...
            %figure ; trisurf(TRI,xint(:),yint(:),etaInt(:)) ; title('eta effective')
