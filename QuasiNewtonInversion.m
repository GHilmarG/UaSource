...
function [Cest,AGlenEst,Info,ub,vb,ud,vd,ubvbLambda,udvdLambda,lx,ly,gamma]=...
                   QuasiNewtonInversion(...
                   Experiment,CtrlVar,MUA,BCs,s,b,h,S,B,ub,vb,ud,vd,ubvbLambda,udvdLambda,alpha,rho,rhow,g,GF,...
                   InvStartValues,Priors,Meas,BCsAdjoint,Info)
    ...
    elseif ~isempty(InvStartValues.InitialSearchStepSize) && ~isnan(InvStartValues.InitialSearchStepSize) && isfinite(InvStartValues.InitialSearchStepSize)
        gamma=InvStartValues.InitialSearchStepSize ;
    else
        gamma=1;
    end
    
    if gamma==0 ; gamma=1 ; end
...
%         CalcMisfitFunction(Experiment,CtrlVar,MUA,BCs,s,b,h,S,B,ub,vb,ud,vd,ubvbLambda,udvdLambda,AGlenEst,Cest,n,m,alpha,rho,rhow,g,GF,Priors,Meas);
     ...
     dJdC=Cest*0;
     F=@(q,ub,vb,ud,vd) CalcMisfitFunction(Experiment,CtrlVar,MUA,BCs,s,b,h,S,B,ub,vb,ud,vd,ubvbLambda,udvdLambda,AGlenEst,Cest-q*dJdC,n,m,alpha,rho,rhow,g,GF,Priors,Meas);
     [J,Idata,IRegC,IRegAGlen,IBarrierC,IBarrierAGlen,ub,vb,ud,vd,ubvbLambda,udvdLambda,dIdu,kv,rh,nlInfo]=F(0,ub,vb,ud,vd);  
    ...
    for iteration=1:nIt
        ...
        F=@(q,ub,vb,ud,vd) CalcMisfitFunction(Experiment,CtrlVar,MUA,BCs,s,b,h,S,B,ub,vb,ud,vd,ubvbLambda,udvdLambda,AGlen0,C0-q*dJdC,n,m,alpha,rho,rhow,g,GF,Priors,Meas);
        [J1,Idata1,IRegC1,IRegAGlen1,IBarrierC1,IBarrierAGlen1,ub,vb,ud,vd,ubvbLambda,udvdLambda,dIdu,kv,rh,nlInfo]=F(gamma,ub,vb,ud,vd);
        ...
        if J1/J0<0.5
            ...
            % dI/dgamma =d I(J0+gamma gradf)= gradf'*gradf
             bb=gamma; fa=J0 ; fb=J1 ; slope0=[]; %slope0=-dJdC'*dJdC;
             
             F=@(q,ub,vb,ud,vd) CalcMisfitFunction(Experiment,CtrlVar,MUA,BCs,s,b,h,S,B,ub,vb,ud,vd,ubvbLambda,udvdLambda,AGlenEst,C0-q*dJdC,n,m,alpha,rho,rhow,g,GF,Priors,Meas);
             nOut=10; listInF=[1:4] ;  listOutF=[7:10];
                
            temp=CtrlVar.InfoLevelBackTrack;
            CtrlVar.InfoLevelBackTrack=CtrlVar.InfoLevelAdjoint;
            [gamma,fgamma,BackTrackingInfoVector,ArgOut{1:nOut-1}]=BackTracking(slope0,bb,fa,fb,F,CtrlVar,nOut,listInF,listOutF,ub,vb,ud,vd);
            CtrlVar.InfoLevelBackTrack=temp;
           
            ub=ArgOut{6} ; vb=ArgOut{7}; ud=ArgOut{8} ; vd=ArgOut{9};
            [J,Idata,IRegC,IRegAGlen,IBarrierC,IBarrierAGlen,ub,vb,ud,vd,ubvbLambda,udvdLambda,dIdu,kv,rh,nlInfo]=F(gamma,ub,vb,ud,vd);
            
            fprintf(' Backtracking returns gamma=%-g and fgamma=%-g, and fgamma/J=%-g \n',gamma,fgamma,fgamma/J)
        ...
        Cest=C0-gamma*dJdC; % update the estimate for C
