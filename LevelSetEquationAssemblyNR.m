function [UserVar,rh,kv]=LevelSetEquationAssemblyNR(UserVar,CtrlVar,MUA,f0,c0,u0,v0,f1,c1,u1,v1)
    ...
    for Iint=1:MUA.nip  % intergration points
        ...
        LT=NG0<1;
        kappaint0=1-1./NG0 ; %  positive diffusion
        kappaint0(LT)=sin(2*pi*NG0(LT))./(2*pi*NG0(LT)+eps) ; % neg, then pos diffusion
        kappaint0=mu.*kappaint0;
        
        LT=NG1<1;
        kappaint1=1-1./NG1 ; %  positive diffusion
        kappaint1(LT)=sin(2*pi*NG1(LT))./(2*pi*NG1(LT)+eps) ; % neg, then pos diffusion
        kappaint1=mu.*kappaint1;
        
        
        
        dkappaUpper=1./NG1.^2;
        dkappaLower=2*pi*cos(2*pi*NG1)./(2*pi*NG1)-2*pi*sin(2*pi*NG1)./(2*pi*NG1+eps).^2 ;
        dkappa=dkappaUpper;
        dkappa(LT)=dkappaLower(LT);
        dkappa=mu.*dkappa;
