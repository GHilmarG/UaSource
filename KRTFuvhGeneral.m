function [R,K,T,F]=KRTFuvhGeneral(u,v,h,S,B,s,b,u0,v0,h0,as,ab,dt,AGlen,n,C,m,coordinates,connectivity,nip,alpha,rho,rhow,g,MeshProp,CtrlVar,...
        nips,etaInt,exx,eyy,exy,Eint)
...
%     end
    ...
    if ~CtrlVar.CisElementBased ;  Cnod=reshape(C(connectivity,1),Nele,nod); end
...
%            tSparse2=toc(tSparse2)    ;    
    ...
    %save File1 Kxu Kxv Kxh Kyu Kyv Kyh Khu Khv Khh

    if ~Ronly ; if any(isnan(K(:))) ; save TestSave  ;  error(' NaN in K ' ) ; end ; end
    if any(isnan(R)) ; save TestSave  ;  error(' NaN in R ' ) ; end
end
