function [R,K,T,F]=KRTFuvh(u,v,h,S,B,u0,v0,h0,as,ab,dt,AGlen,n,C,m,coordinates,connectivity,nip,alpha,rho,rhow,g,MeshProp,CtrlVar)
...
for Iint=1:nip
    ...
    rhoint=rhonod*fun;
    varrhogint=g*rhoint.*(1-rhoint/rhow);
    ...
    kH=CtrlVar.kH;
    Heint = HeavisideApprox(kH,hint-hfint,CtrlVar.Hh0);  % very important to calculate Heint and deltaint in consistent manner
    ...
    for Inod=1:nod
        if ~Ronly
            for Jnod=1:nod
                ...
                Kxh(:,Inod,Jnod)=Kxh(:,Inod,Jnod)...
                    ...
                    +g*rhoint.*(Heint+hint.*deltaint).*((rhoint.*dhdx/rhow-dHdx)*ca-sa).*fun(Inod).*fun(Jnod)...
                    +(g*rhoint.*rhoint/rhow).*hint.*Heint.*ca.*fun(Inod).*Deriv(:,1,Jnod)...
                    -varrhogint.*hint.*ca.*Deriv(:,1,Inod).*fun(Jnod)).*detJw;
                ...
                Kyh(:,Inod,Jnod)=Kyh(:,Inod,Jnod)...
                    ...
                    +g*rhoint.*(Heint+hint.*deltaint).*(rhoint.*dhdy/rhow-dHdy).*ca.*fun(Inod).*fun(Jnod)...
                    +(g*rhoint.*rhoint/rhow).*hint.*Heint.*ca.*fun(Inod).*Deriv(:,2,Jnod)...
                    -varrhogint.*hint.*ca.*Deriv(:,2,Inod).*fun(Jnod)).*detJw;
        ...
        t2=0.5*ca*varrhogint.*hint.^2.*Deriv(:,1,Inod);
        ...
        t2=0.5*ca*varrhogint.*hint.^2.*Deriv(:,2,Inod);
