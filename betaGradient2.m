function [betaGrad]=betaGradient2(u,v,Lambda,Mu,C,connectivity,coordinates,nip);
    ...
    for Iele=1:Nele
        ...
        for Iint=1:nip                           % loop over integration points
            ...
            for Inod=1:nod
                ...
                CI=C_l(Inod)*fun(Inod) ;           %CI=C_l'*fun ;
                betaI= sqrt(1/CI) ;
                uI=u_l(Inod)*fun(Inod) ;           %   uI=u_l'*fun ;
                vI=v_l(Inod)*fun(Inod) ;           % vI=v_l'*fun ;
                LambdaI=Lambda_l(Inod)*fun(Inod);  % LambdaI=Lambda_l'*fun;
                MuI=Mu_l(Inod)*fun(Inod) ;         %  MuI=Mu_l'*fun;
                betaGradI(Inod)=betaGradI(Inod)-2*(LambdaI*uI+MuI*vI)*betaI*detJw;
